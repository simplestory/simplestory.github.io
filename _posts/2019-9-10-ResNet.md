...
其中$x_l$，$x_{l+1}$为第$l$个残差单元的输入和输出，$\mathcal{F}$为残差函数，$f$为$ReLU$激活函数，
$\mathcal{W}_l=\{W_{l,k}\vert 1\le k\le K\}$是第$l$个单元的权重（包含偏差值），$K$是残差单元中的网络层数。
...
递归可得
（$x_{l+2} = x_{l+1}+\mathcal{F}(x_{l+1},\mathcal{W}_{l+1}) = x_l+\mathcal{F}(x_l,\mathcal{W}_l)+\mathcal{F}(x_{l+1},\mathcal{W}_{l+1})$等）：
